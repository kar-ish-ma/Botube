...
const registerUser = asyncHandler(async (req, res) => {
  ...
  console.log("After cloudinary", avatar);
...
const generateAccessTokenAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(200,"Something went wrong wrong while generating refresh token");
  }
};
const loginUser = asyncHandler(async(req,res)=>{
    const {email,username,password}= req.body
    if(!username || !email){
        throw new ApiError(400,"Email and username needed to login ")
    }
...
const updateAccountDetails = asyncHandler(async(req,res)=>{
  ...
  .status(200)
  .json (
    new ApiResponse(200,"Api Response Updated successfully")
  )
})
